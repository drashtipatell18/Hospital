...
@section('content')
    <div class="content">
        ...
        <div class="row">
            <div class="col-lg-8 offset-lg-2">
                <form id="userForm" action="{{ isset($users) ? url('/user/update/' . $users->id) : url('/user/insert') }}"
                    ...
                    <div class="row">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="role_id">
                                    ...
                                    @foreach ($roles as $id => $name)
                                        <option value="{{ $id }}"
                                            {{ (old('role_id') ?? ($users->role_id ?? '')) == $id ? 'selected' : '' }}>
                                            {{ $name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="profile-upload">
                                    <div class="upload-img">
                                        ...
                                        src="{{ isset($users->image) ? asset('users/' . $users->image) : asset('assets/img/user.jpg') }}">                                    
                                    </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="status" id="user_active"
                                        value="active"
                                        {{ old('status', isset($users) && $users->status ? $users->status : '') == 'active' ? 'checked' : '' }}>
                                ...
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="status" id="user_inactive"
                                        value="inactive"
                                        {{ old('status', isset($users) && $users->status ? $users->status : '') == 'inactive' ? 'checked' : '' }}>
                        ...
                        <div class="col-12 text-center m-t-20">
                            <button
                                class="btn btn-primary submit-btn">{{ isset($users) ? 'Update User' : 'Create User' }}</button>
                        </div>
                    </div>
                </form>
            </div>
